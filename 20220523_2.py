...
# 응용 A
#
# ser 리스트의 1~5까지 누적해서 더한 결과 값을 출력하기
ser = [1, 2, 3, 4, 5]
sssss = 0
for i in ser:
    sssss+=i
...
# 응용 A
#
# ser 리스트의 1~5까지 누적해서 곱한 결과 값을 출력하기
ser = [1, 2, 3, 4, 5]
sssss = 1 # 0으로 정하면 값이 0이 나온다, 따라서 1로 초기값을 줘야 한다 (누적을 이유로)
for i in ser:
    sssss*=i
    print (sssss)
#
...
for i in range(10): # 0부터 (10-1)까지 출력된다
    print (i)

for i in range(1, 10): # 1부터 (10-1)까지 출력된다
    print (i)

for i in range(2, 10): # 2부터 (10-1)까지 출력된다
    print (i)

#
# 전부 값이 1씩 증가한다

for i in range(1, 10, 2): # 1부터 (10-1)까지 , 값이 2씩 증가하여 출력된다
    print (i)
...
for i in range(5):
    print (str(i), "目") # 문자열의 결합! ex) '1' + '3' = '13'


aaaaa = 10
bbbbb = "asdfghjkl;" # %d = 정수로 출력하라는 의미
print("%d, %a"%(aaaaa, bbbbb))
...
for i in reversed(range(10)): # (10-1)번째부터 1번째까지 역방향됨(Reversed)
    # 出力
